...
def brute(input_xored, expected_output, key_len):
    ...
    if len(input_xored) != len(expected_output):
        print
        '[!] Input xored and expected output lengths not match!'
        return False
    ...
    for i in range(key_len):
        ...
        if not found:
            print
            '[!] Could not found partial key value.'
            break
...
def main(argv):
    ...
    if len(cipher) != len(plain):
        print
        '[!] Cipher text and plain text must be of same length!'
        return False

    if len(cipher) % keylen != 0:
        print
        '[!] Cipher text and plain text lengths must be divisble by keylen!'
        return False
    ...
    if status:
        ...
        '\tKey:\t\t\t', str(key)
        print
        '\tDecrypted string:\t' + xorstring(cipher, key)
    else:
        print
        '[!] Key not found.'
